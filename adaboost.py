...
def train_weak_classifier(parameter,inst_weight,class_label,feature,bool_label):
    ...
    color1=random.randint(0,2)   #Randomly selecting color
    ...
    trueCase=index[np.where(feature[color1][index,p1]>feature[color2][index,p2])]
    #Cases where the condition of decision stump is not satisfied
    falseCase=index[np.where(feature[color1][index,p1]<=feature[color2][index,p2])]
    ...
    parameter+=[[p1,p2,color1,color2,class_label,hypo_weight]]
        ...
        for j in range(c):
            for k in range(4):
                if test[parameter[j+c*k][2]][i][parameter[j+c*k][0]]>\
                   test[parameter[j+c*k][3]][i][parameter[j+c*k][1]]:
                    final[k]-=parameter[j+c*k][5]
                else:
                    final[k]+=parameter[j+c*k][5]
